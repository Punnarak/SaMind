  ...
  try {
    ...
    const transformedResult = await Promise.all(
      result.rows.map(async (row, index) => {
        ...
        const patientData = {
          no: (index + 1).toString().padStart(2, "0"), // Two-digit format
          patientId: `PID${patientId}`, // Prefixing with "PID"
          name: `${row.fname} ${row.lname}`,
          age: row.age,
        };
        ...
        // Get mood data for the patient
        const moodData = await getAvatarMoodDetection(patientId);
        patientData.mood = moodData
          ? getHighestMood(moodData.avatarMoodDetec)
          : "-";
...
//     const transformedResult = {
//       avatarMoodDetec: {
//         positive: result.rows[0].positive_percent,
//         negative: result.rows[0].nagative_percent,
//         neutral: result.rows[0].neutral_percent,
//       },
//     };
...
async function getAvatarMoodDetection(patientId) {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return null; // Return null when no mood data is available
    }
...
router.post('/checkgamepatient', async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      ...
      query = 'INSERT INTO gamedoctor (patient_id, goodword, timeplay, sad, normal, happy) VALUES ($1, $2, $3, $4, $5, $6)';
      queryParams = [cleanedPatientId, 0, 0, 0, 0, 0];
